...
class EduNewsScraper:
    ...
    def filter_by_unit(self, news_items, target_unit):
        ...
        for item in news_items:
            # 在單位或標題中搜尋目標單位
            if (target_unit in item.get('unit', '') or 
                target_unit in item.get('title', '')):
    ...
    def scrape_news(self, target_unit, max_count):
        ...
        while len(scraped_news) < max_count and page <= max_pages:
            ...
            for item in filtered_items:
                ...
                print(f"\n處理新聞 {len(scraped_news)+1}: {item['title'][:40]}...")
                
                # 獲取詳細內容（可選，如果不需要聯絡人資訊可以跳過）
        ...
        print(f"\n🎉 成功爬取 {len(scraped_news)} 筆 {target_unit} 的新聞")
        return scraped_news
...
def get_user_input():
    ...
    while True:
        try:
            # 輸入文章數量
            count_input = input("請輸入要爬取的文章數量 (1-20): ").strip()
            if not count_input.isdigit():
                print("❌ 請輸入有效的數字！")
                continue
            
            count = int(count_input)
            if count < 1 or count > 20:  # 減少到20篇
                print("❌ 文章數量必須在 1-20 之間！")
                continue
            
            # 輸入發布單位
            print("\n可選的發布單位: 體育署、國教署、青年署、高教司、技職司、學特司等")
